...
if $ipCMD2 l show v-dn42 &>/dev/null || $ipCMD1 l show v-chat &>/dev/null; then
    echo "veth pair already exists, removing for clean recreation..."
    $ipCMD2 l del v-dn42 2>/dev/null || true
    $ipCMD1 l del v-chat 2>/dev/null || true
fi

ip l add v-dn42 netns $pid2 type veth peer v-chat netns $pid1
if [ $? -ne 0 ]; then
...
$ipCMD1 l set v-chat vrf vrf42
$ipCMD1 l set v-chat up
$ipCMD1 a flush scope link dev v-chat
$ipCMD1 a add fe80::1/64 dev v-chat
$ipCMD1 route replace $DN42_ULA_ADDR/128 via fe80::2 dev v-chat vrf vrf42
